...
use crate::{
  common, Complex, CEPS_MEM, FRAME_SIZE, FREQ_SIZE, NB_BANDS, NB_DELTA_CEPS,
...
impl DenoiseFeatures {
  ...
  fn find_pitch(&mut self) -> usize {
    let input = &self.input_mem
      [self.input_mem.len().checked_sub(PITCH_BUF_SIZE).unwrap()..];
  ...
  pub fn compute_frame_features(&mut self) -> bool {
    ...
    self.features[NB_BANDS + 3 * NB_DELTA_CEPS] =
      0.01 * (pitch_idx as f32 - 300.0);
    let mut log_max = -2.0;
    let mut follow = -2.0;
    let mut e = 0.0;
    for i in 0..NB_BANDS {
    ...
    for i in 0..NB_DELTA_CEPS {
      ...
      self.features[NB_BANDS + NB_DELTA_CEPS + i] =
        ceps_0[i] - 2.0 * ceps_1[i] + ceps_2[i];
    }
    ...
    self.features[NB_BANDS + 3 * NB_DELTA_CEPS + 1] =
      spec_variability / CEPS_MEM as f32 - 2.1;
