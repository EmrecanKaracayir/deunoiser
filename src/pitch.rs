use crate::{
  PITCH_BUF_SIZE, PITCH_FRAME_SIZE, PITCH_MAX_PERIOD, PITCH_MIN_PERIOD,
};
  ...
  fn remove_doubling(&mut self, pitch_idx: usize) -> (usize, f32) {
    ...
    for k in 2..=15 {
      ...
      let cont = if (t1 as isize - prev_period as isize).abs() <= 1 {
        self.last_gain
      } else if (t1 as isize - prev_period as isize).abs() <= 2
        && 5 * k * k < t0
      {
        self.last_gain / 2.0
      } else {
    ...
    for k in 0..3 {
      xcorr[k] =
        inner_prod(&x[max_period..], &x[(max_period - (t + k - 1))..], n);
    }
...
fn fir5_in_place(xs: &mut [f32], num: &[f32]) {
  ...
  for x in xs {
    let out =
      *x + num0 * mem0 + num1 * mem1 + num2 * mem2 + num3 * mem3 + num4 * mem4;
    mem4 = mem3;
    mem3 = mem2;
    mem2 = mem1;
    mem1 = mem0;
