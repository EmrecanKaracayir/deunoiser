...
use deunoiser::util::zip3;
use deunoiser::util::Biquad;
use deunoiser::DenoiseFeatures;
use deunoiser::{NB_BANDS, NB_FEATURES};

// After this many frames, we re-randomize the gains and the filters.
const GAIN_CHANGE_COUNT: u32 = 2821;

fn glob_paths<'a>(
  globs: impl Iterator<Item = &'a str>,
) -> Result<Vec<PathBuf>> {
  let mut ret = Vec::new();
  for glob in globs {
    let paths =
      glob::glob(glob).context(format!("while expanding glob {}", glob))?;
    for p in paths {
...
fn main() -> Result<()> {
  let matches = Command::new("deunoiser-gen-training-data")
    .version(crate_version!())
    .about("Generate data for training deunoiser models")
    .arg(
      Arg::new("signal-glob")
  ...
  let out_file =
    hdf5::File::create(out_name).context("failed to open output file")?;
...
impl SignalReader {
  ...
  fn next_reader(&mut self) -> Result<()> {
    ...
    let num_samples = deunoiser::FRAME_SIZE * self.frames_per_file;
    if len > num_samples {
      reader
      ...
      self.frames_left = len / deunoiser::FRAME_SIZE;
    }
...
impl NoiseSimulator {
  fn new(signal: SignalReader, noise: SignalReader) -> NoiseSimulator {
    NoiseSimulator {
      ...
      signal_gain: 1.0,
      noise_gain: 1.0,
      lowpass: deunoiser::FREQ_SIZE,
      band_lp: deunoiser::NB_BANDS - 1,

      sig_buf: vec![0.0; deunoiser::FRAME_SIZE],
      noise_buf: vec![0.0; deunoiser::FRAME_SIZE],
      out_buf: vec![0.0; deunoiser::FRAME_SIZE],
  ...
  fn randomize(&mut self) {
    ...
    */
    ...
    self.lowpass = (deunoiser::FREQ_SIZE as f32 * 3000.0 / 24000.0
      * 50.0_f32.powf(rng.gen())) as usize;

    self.band_lp = deunoiser::EBAND_5MS
      .iter()
      .position(|x| x << deunoiser::FRAME_SIZE_SHIFT > self.lowpass)
      .unwrap_or(deunoiser::NB_BANDS - 1);
  }
