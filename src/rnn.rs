...
/// `deunoiser` has a built-in model that should work for most purposes, but if you have
...
impl RnnModel {
  ...
  /// Reads an `RnnModel` from an array of bytes, in our new deunoiser format.
  ///
  ...
  fn from_bytes_impl<'a>(
    ...
    let read_array =
      |bytes: &'a [i8], len: usize| -> Option<(Cow<'static, [i8]>, &[i8])> {
        if bytes.len() >= len {
    ...
    let read_dense = |bytes: &'a [i8]| -> Option<(DenseLayer, &[i8])> {
      ...
      let (input_weights, bytes) =
        read_array(&bytes[3..], nb_neurons * nb_inputs)?;
    ...
    let read_gru = |bytes: &'a [i8]| -> Option<(GruLayer, &[i8])> {
      ...
      let activation = act(bytes[2])?;
      let (input_weights, bytes) =
        read_array(&bytes[3..], 3 * nb_neurons * nb_inputs)?;
      let (recurrent_weights, bytes) =
        read_array(bytes, 3 * nb_neurons * nb_neurons)?;
    ...
    if input_dense.nb_neurons != vad_gru.nb_inputs
...
impl<'model> RnnState<'model> {
  ...
  pub fn compute(&mut self, gains: &mut [f32], vad: &mut [f32], input: &[f32]) {
    ...
    copy(
      &mut denoise_buf
        [(model.vad_gru.nb_neurons + model.noise_gru.nb_neurons)..],
      input,
    );
