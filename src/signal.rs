...
/// # Example
/// ```rust
/// use deunoiser::dasp::signal::{self, Signal};
/// use deunoiser::dasp::sample::Sample;
/// use deunoiser::DenoiseSignal;
///
...
impl<'model, S: Signal> DenoiseSignal<'model, S> {
  ...
  ) -> DenoiseSignal<'model, S> {
    DenoiseSignal {
      input,
      states: vec![
        DenoiseState::from_model_owned(Cow::Borrowed(model));
        S::Frame::CHANNELS
      ],
      in_bufs: vec![[0.0; FRAME_SIZE]; S::Frame::CHANNELS],
      out_bufs: vec![[0.0; FRAME_SIZE]; S::Frame::CHANNELS],
  ...
  pub fn from_model(input: S, model: RnnModel) -> DenoiseSignal<'static, S> {
    DenoiseSignal {
      input,
      states: vec![
        DenoiseState::from_model_owned(Cow::Owned(model));
        S::Frame::CHANNELS
      ],
      in_bufs: vec![[0.0; FRAME_SIZE]; S::Frame::CHANNELS],
      out_bufs: vec![[0.0; FRAME_SIZE]; S::Frame::CHANNELS],
  ...
  fn refill_out_bufs(&mut self) -> bool {
    ...
    for i in 0..FRAME_SIZE {
      for (ch, samp) in
        self.input.next().to_float_frame().channels().enumerate()
      {
    ...
    for ch in 0..S::Frame::CHANNELS {
      self.states[ch]
        .process_frame(&mut self.out_bufs[ch][..], &self.in_bufs[ch][..]);
    }
