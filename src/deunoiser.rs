...
use deunoiser::{DenoiseState, RnnModel};
...
impl<W: Write> FrameWriter for RawFrameWriter<W> {
  fn write_frame(&mut self, buf: &[f32]) -> Result<(), Error> {
    ...
    for (dst, src) in self.buf.chunks_mut(2).zip(buf) {
      let bytes = (src.max(i16::MIN as f32).min(i16::MAX as f32).round()
        as i16)
        .to_le_bytes();
      dst[0] = bytes[0];
      dst[1] = bytes[1];
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let out_file =
    BufWriter::new(File::create(out_name).with_context(|| {
      format!("Failed to open output file \"{}\"", out_name)
    })?);
  let in_wav = matches.is_present("wav-in")
    || Path::new(in_name).extension() == Some("wav".as_ref());
  let out_wav = matches.is_present("wav-out")
    || Path::new(out_name).extension() == Some("wav".as_ref());
  ...
  let model = if let Some(model_path) = matches.value_of("model") {
    let data =
      std::fs::read(model_path).context("Failed to open model file")?;
