...
/// It should be avoided, use directly `rnnoise_create`
#[no_mangle]
pub unsafe extern "C" fn rnnoise_init(
  st: *mut DenoiseState,
  model: *mut RNNModel,
) -> c_int {
  let state = if model.is_null() {
    ...
    crate::DenoiseState::from_model_owned(Cow::Borrowed(
      &*(model as *mut crate::RnnModel),
    ))
  };
...
#[no_mangle]
pub unsafe extern "C" fn rnnoise_create(
  model: *mut RNNModel,
) -> *mut DenoiseState {
  let state = if model.is_null() {
    crate::DenoiseState::default()
...
pub unsafe extern "C" fn rnnoise_process_frame(
  ...
  let output =
    std::slice::from_raw_parts_mut(out, crate::DenoiseState::FRAME_SIZE);
  let input =
    std::slice::from_raw_parts(input, crate::DenoiseState::FRAME_SIZE);
...
#[no_mangle]
pub unsafe extern "C" fn rnnoise_model_from_file(
  file: *mut FILE,
) -> *mut RNNModel {
